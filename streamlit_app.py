...
def _build_diff_markup(original_text: str, edited_text: str) -> Tuple[str, str]:
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        original_segment = "".join(
            html.escape(token) for token in original_tokens[i1:i2]
        )
...
def _build_similarity_highlight(
    ...
    def build_ngrams(words: List[str], size: int) -> set[str]:
        return {
            " ".join(words[idx : idx + size]) for idx in range(len(words) - size + 1)
        }
...
def _render_similarity_panels(
    source_text: str, ai_text: str, rewrite_text: str
) -> None:
...
def _build_repair_suggestions(
    ...
    if metric_label == "AI-ism Likelihood":
        ...
        if phrases:
            suggestions.append("Rewrite or remove these phrases: " + ", ".join(phrases))
            suggestions.append(
                "Use direct verbs and concrete nouns over scaffolding clauses."
            )
        else:
            suggestions.append(
                "Reduce template phrases and tighten transitions between ideas."
            )
        ...
        suggestions.append(
            "Vary verbs and adjectives that appear in consecutive sentences."
        )
    elif metric_label == "Syntactic Complexity":
        suggestions.append(
            "Add one subordinate clause to key sentences to reflect your original structure."
        )
        suggestions.append(
            "Balance simple statements with one or two layered sentences per paragraph."
        )
    elif metric_label == "Function Word Ratio":
        suggestions.append("Trim filler words such as 'that', 'just', and 'really'.")
        suggestions.append(
            "Prefer precise nouns and verbs over connector-heavy phrasing."
        )
        ...
        suggestions.append(
            f"Reduce transition markers (current count: {marker_count})."
        )
        suggestions.append(
            "Keep one transition per paragraph, and let sentence order do the work."
        )
        ...
        suggestions.append(
            f"Add concrete nouns or numbers to raise content words (now {content_words})."
        )
        suggestions.append(
            "Replace abstract fillers with specific examples or observations."
        )
        ...
        suggestions.append(
            f"Reintroduce cautious wording where appropriate (hedges: {headges})."
        )
        suggestions.append(
            "Balance certainty with qualifiers like 'likely' or 'suggests'."
        )
    else:
        suggestions.append(
            "Review highlighted changes and restore any phrasing that sounds less like you."
        )
    return suggestions
    ...
    if local_storage:
        try:
            local_storage.deleteItem(
                HUMAN_TEXT_KEY, key=_next_storage_key("delete_human")
            )
        except KeyError:
            pass
    st.session_state.analysis = None
...
def clear_source(local_storage: LocalStorage | None) -> None:
    ...
    if local_storage:
        try:
            local_storage.deleteItem(
                SOURCE_TEXT_KEY, key=_next_storage_key("delete_source")
            )
        except KeyError:
            pass
    st.session_state.analysis = None
...
def auto_load_text(
    ...
    if not local_storage:
        st.session_state.auto_load_notice = (
            "Autosave is unavailable because streamlit-local-storage is missing."
        )
...
def apply_preset(local_storage: LocalStorage | None, preset_index: int) -> None:
    ...
    _set_storage_item(
        local_storage, PARAPHRASE_TEXT_KEY, preset["paraphrase"], "preset_paraphrase"
    )
...
def render_header() -> None:
    ...
    with header_left:
        if LOGO_PATH.exists():
            ...
            st.markdown(
                "<div class='vt-logo-fallback'>VT</div>", unsafe_allow_html=True
            )
    with header_right:
        st.markdown(
            """
...
def render_upload_screen(local_storage: LocalStorage | None) -> None:
    ...
    st.markdown(
        "<div class='vt-section-title'>Configuration</div>", unsafe_allow_html=True
    )
    ...
    with config_left:
        ...
        if (
            st.session_state.calibration_toggle_prev
            != st.session_state.calibration_toggle
        ...
        if st.session_state.use_default_standards:
            ...
            if st.session_state.defaults_reset_notice:
                st.success(
                    "Default thresholds restored. The next analysis uses standard settings."
                )
                st.session_state.defaults_reset_notice = False
        if not st.session_state.use_default_standards:
    ...
    st.markdown(
        "<div class='vt-section-title'>Text Inputs</div>", unsafe_allow_html=True
    )
    ...
    with section_top_left:
        st.markdown(
            "<div class='vt-card-title'>Section 1: Human Baseline (Unassisted)</div>",
            unsafe_allow_html=True,
        )
        ...
        with human_actions_right:
            st.button(
                "Clear Section 1",
                on_click=lambda: clear_human(local_storage),
                use_container_width=True,
            )

    with section_top_right:
        st.markdown(
            "<div class='vt-card-title'>Section 2: Source Material (Upload Only)</div>",
            unsafe_allow_html=True,
        )
        ...
        with source_actions_right:
            st.button(
                "Clear Section 2",
                on_click=lambda: clear_source(local_storage),
                use_container_width=True,
            )
    ...
    with section_bottom_left:
        st.markdown(
            "<div class='vt-card-title'>Section 3: AI-Generated Draft</div>",
            unsafe_allow_html=True,
        )
        st.text_input(
        ...
        with ai_actions_right:
            st.button(
                "Clear Section 3",
                on_click=lambda: clear_ai(local_storage),
                use_container_width=True,
            )

    with section_bottom_right:
        st.markdown(
            "<div class='vt-card-title'>Section 4: Writer Paraphrase (Your Rewrite)</div>",
            unsafe_allow_html=True,
        )
        ...
        if (
            upload_paraphrase
            and upload_paraphrase.name != st.session_state.paraphrase_file_name
        ):
            try:
        ...
        with paraphrase_actions_left:
            st.button(
                ...
                args=(
                    ...
                    "paraphrase_text",
                    "paraphrase",
                    "paraphrase_file_name",
                ),
            )
        with paraphrase_actions_right:
            st.button(
                "Clear Section 4",
                on_click=lambda: clear_paraphrase(local_storage),
                use_container_width=True,
            )
...
def render_dashboard_screen() -> None:
    ...
    st.markdown(
        "<div class='vt-section-title'>Analysis View</div>", unsafe_allow_html=True
    )
    section = st.radio(
        "",
        options=[
    ...
    if section == "Executive Summary":
        st.markdown(
            "<div class='vt-section-title'>Executive Summary</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"""
        ...
        st.markdown(
            "<div class='vt-card-caption'>Voice Preservation Gauge</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
        ...
        st.markdown(
            "<div class='vt-section-title'>Component Breakdown</div>",
            unsafe_allow_html=True,
        )
        components = {
        ...
        for label, value in components.items():
            st.markdown(
                f"<div class='vt-card-caption'>{label}</div>", unsafe_allow_html=True
            )
            st.progress(min(max(int(value), 0), 100))
        st.markdown(
            "<div class='vt-section-title'>Quick Stats</div>", unsafe_allow_html=True
        )
        stats = [
        ...
        st.markdown(
            "<div class='vt-section-title'>Similarity Index</div>",
            unsafe_allow_html=True,
        )
        ...
        with sim_left:
            _render_similarity_metric(
                "Source vs rewrite (cosine)", source_similarity_cosine, "#16a34a"
            )
            _render_similarity_metric(
                "Source vs rewrite (n-gram)", source_similarity_ngram, "#16a34a"
            )
        with sim_right:
            _render_similarity_metric(
                "AI vs rewrite (cosine)", ai_similarity_cosine, "#ef4444"
            )
            _render_similarity_metric(
                "AI vs rewrite (n-gram)", ai_similarity_ngram, "#ef4444"
            )
        st.markdown(
        ...
        if analysis.score < 80 and st.button(
            "Open Repair Preview", use_container_width=True
        ):
        ...
        st.markdown(
            "<div class='vt-section-title'>Metric Deep-Dive</div>",
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            build_radar_chart(
        ...
        st.markdown(
            "<div class='vt-section-title'>Data Only</div>", unsafe_allow_html=True
        )
        st.markdown(
            "<div class='vt-muted'>All metrics listed without charts.</div>",
            unsafe_allow_html=True,
        )
        ...
        def _render_card(
            ...
            value_text = format_metric(value)
            badge_html = ""
            if tone:
                badge_html = f"<span class='vt-badge'>{html.escape(tone)}</span>"
            caption_html = (
                f"<div class='vt-card-caption'>{html.escape(caption)}</div>"
                if caption
                else ""
            )
            st.markdown(
                f"""
				<div class="vt-card vt-subtle">
				  <div class="vt-card-title">{html.escape(title)} {badge_html}</div>
				  <div class="vt-card-value">{html.escape(value_text)}</div>
				  {caption_html}
				</div>
				""",
                unsafe_allow_html=True,
        ...
        overall_left, overall_mid, overall_right = st.columns(3)
        with overall_left:
            _render_card(
                "Voice Preservation Score",
                analysis.score,
                "AI Source vs Writer Rewrite",
            )
        with overall_mid:
            _render_card("Classification", analysis.classification)
        with overall_right:
            _render_card("Consistency Score", analysis.consistency_score)
        ...
        for idx, (label, score) in enumerate(components):
            with comp_cols[idx]:
                _render_card(label, score)
        ...
        stats = [
            ...
            ("Sentence Delta", analysis.sentence_delta, "Structure change"),
            ("AI-isms", analysis.ai_ism_total, "Writer rewrite phrases"),
            ("Original word count", analysis.original_word_count, None),
            ("Rewrite word count", analysis.edited_word_count, None),
            ("Original sentence count", analysis.original_sentence_count, None),
            ("Rewrite sentence count", analysis.edited_sentence_count, None),
        ]
        stat_cols = st.columns(4)
        for idx, (label, value, caption) in enumerate(stats):
            with stat_cols[idx % 4]:
                _render_card(label, value, caption)
        ...
        for idx, (label, value) in enumerate(similarity_rows):
            with sim_cols[idx]:
                _render_card(label, value)

        st.markdown(
            "<div class='vt-section-title'>Metric Table (as cards)</div>",
            unsafe_allow_html=True,
        )
        for metric in METRICS:
            label = metric["label"]
            ...
            verdict = analysis.metric_results[key].verdict
            normalized_score = analysis.metric_results[key].normalized_score
            left, mid, right, extra = st.columns([2.2, 1.1, 1.1, 1.4])
            with left:
                st.markdown(
                    f"""
					<div class="vt-card vt-subtle">
					  <div class="vt-card-title">{html.escape(label)} <span class='vt-badge'>{html.escape(verdict)}</span></div>
					  <div class="vt-card-caption">{html.escape(metric['caption'])}</div>
					</div>
					""",
                    unsafe_allow_html=True,
                )
            with mid:
                _render_card("AI Source", original_value)
            with right:
                _render_card("Writer Rewrite", edited_value)
            with extra:
                _render_card(
                    "Delta / Normalized",
                    f"{format_metric(delta)} / {format_metric(normalized_score)}",
                )
        ...
        st.markdown(
            "<div class='vt-section-title'>Text Comparison</div>",
            unsafe_allow_html=True,
        )
...
def render_repair_preview() -> None:
    ...
    # Removed UI slider ("Workspace height") per request and use a fixed height.
    panel_height = 360

    # Ensure the three panel header cards align to the same height.
    st.markdown("<div class='vt-repair-panel-headers'>", unsafe_allow_html=True)
    left_col, mid_col, right_col = st.columns(3, gap="large")

    with left_col:
        st.markdown(
            "<div class='vt-card vt-subtle vt-panel-header'><div class='vt-card-title'>AI Draft (Original)</div><div class='vt-card-caption'>Baseline draft generated by the AI.</div></div>",
            unsafe_allow_html=True,
        )
        st.text_area(
            "AI draft",
    ...
    with mid_col:
        st.markdown(
            "<div class='vt-card vt-subtle vt-panel-header'><div class='vt-card-title'>Your Paraphrase (Potential AI Pattern Carryover)</div><div class='vt-card-caption'>Your rewrite of the AI draft; may still mimic AI style patterns.</div></div>",
            unsafe_allow_html=True,
        )
        st.text_area(
            "Your paraphrase",
        ...
        st.caption(
            f"Metric score ({metric_focus}): {edited_score:.1f}  (Δ {edited_score - original_score:+.1f})"
        )

    with right_col:
        st.markdown(
            "<div class='vt-card vt-subtle vt-panel-header'><div class='vt-card-title'>Negotiated Rewrite (Your Choice)</div><div class='vt-card-caption'>Pick an option or write a custom version. The goal is to reduce mimicry while keeping meaning.</div></div>",
            unsafe_allow_html=True,
        )

        # --- Distinct option generators (lightweight + deterministic) ---
        def _clean_spacing(text: str) -> str:
            text = re.sub(r"[ \t]+", " ", text)
            text = re.sub(r"\n{3,}", "\n\n", text)
            return text.strip()

        def _option_a_balanced(ai_text: str, paraphrase_text: str) -> str:
            """Balanced compromise: keep paraphrase content but reduce common AI scaffolding."""
            text = paraphrase_text or ""
            text = re.sub(
                r"\b(In this (essay|paper|study),\s*)?it is important to note that\b\s*",
                "",
                text,
                flags=re.IGNORECASE,
            )
            text = re.sub(
                r"\b(Moreover|Furthermore|Consequently|Accordingly|Therefore|Thus),\s*(Moreover|Furthermore|Consequently|Accordingly|Therefore|Thus)\b",
                r"\1",
                text,
                flags=re.IGNORECASE,
            )
            text = re.sub(
                r"\b(very|really|clearly|obviously)\b\s*",
                "",
                text,
                flags=re.IGNORECASE,
            )
        ...
        def _option_b_more_human_like(human_text: str, paraphrase_text: str) -> str:
            """More human-like: add mild hedging + first-person framing (if absent) and reduce formality."""
            text = paraphrase_text or ""
            lowered = text.lower()
            has_first_person = bool(re.search(r"\b(i|my|we|our)\b", lowered))
            has_hedge = bool(
                re.search(r"\b(might|may|likely|suggests?|appears?|seems?)\b", lowered)
            )
            if not has_first_person:
                framing = "In my view, "
                if (human_text or "").strip().lower().startswith("i "):
                    framing = "I think "
        ...
        # Build negotiated text BEFORE rendering the scrollable box.
        mode = st.session_state.repair_choice_mode
        if mode == "Option C (Accept AI draft)":
            negotiated_text = st.session_state.ai_text
        elif mode == "Option B (More human-like)":
            negotiated_text = _option_b_more_human_like(
                st.session_state.human_text,
                st.session_state.paraphrase_text,
            )
        elif mode == "Option A (Balanced)":
            negotiated_text = _option_a_balanced(
                st.session_state.ai_text,
                st.session_state.paraphrase_text,
            )
        else:
        ...
        # Scrollable negotiated window first (same height as the other panels)
        if mode == "Custom":
        ...
        # Options MUST be below the scrollable window (requested)
...
def render_calibration(local_storage: LocalStorage | None) -> None:
    st.markdown(
        "<div class='vt-section-title'>Calibration Panel</div>", unsafe_allow_html=True
    )
    if st.session_state.use_default_standards:
    ...
    for metric in METRICS:
        ...
        with st.expander(label):
            ...
            if st.button(f"Reset {label}", key=f"reset_{key}"):
                ...
                st.rerun()
    st.markdown(
        "<div class='vt-section-title'>Impact Preview</div>", unsafe_allow_html=True
    )
    st.markdown(
...
def render_settings() -> None:
    ...
    st.selectbox(
        "Default Calibration Strictness",
        options=["Strict", "Moderate", "Permissive"],
        index=1,
    )
...
with st.sidebar:
    st.markdown(
        "<div class='vt-section-title'>Navigation</div>", unsafe_allow_html=True
    )
    page = st.radio(
        "",
        options=[
